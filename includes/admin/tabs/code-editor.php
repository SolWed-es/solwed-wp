    ...
    <div class="solwed-code-editor-container">
        ...
        <div class="solwed-code-editor-main">
            <form method="post" action="">
                ...
                <div class="solwed-form-section">
                    <h2><?php _e('👨‍💻 Editor de Código Avanzado', 'solwed-wp'); ?></h2>
                    <p class="description"><?php _e('Añade CSS, JavaScript y PHP personalizado a tu sitio web. Usa esta funcionalidad con precaución.', 'solwed-wp'); ?></p>
                    
                    <!-- CSS Personalizado -->
                    <div class="solwed-code-editor-wrapper">
                        <h3><?php _e('CSS Personalizado (Normal)', 'solwed-wp'); ?></h3>
                        <p class="description"><?php _e('CSS que se carga con prioridad normal en wp_head.', 'solwed-wp'); ?></p>
                    ...
                    <div class="solwed-code-editor-wrapper">
                        <h3><?php _e('CSS de Alta Prioridad', 'solwed-wp'); ?></h3>
                        <p class="description"><?php _e('CSS que se carga con máxima prioridad y sobrescribe otros estilos. Se ejecuta después de todos los demás CSS.', 'solwed-wp'); ?></p>
                    ...
                    <div class="solwed-code-editor-wrapper">
                        <h3><?php _e('JavaScript Personalizado', 'solwed-wp'); ?></h3>
                        <p class="description"><?php _e('JavaScript que se carga en el footer. Se ejecuta cuando el DOM esté listo.', 'solwed-wp'); ?></p>
                    ...
                    <!-- PHP Personalizado -->
                    <div class="solwed-code-editor-wrapper">
                        <h3><?php _e('PHP Personalizado', 'solwed-wp'); ?> <span style="color: #d63638; font-weight: bold;"><?php _e('(¡PELIGROSO!)', 'solwed-wp'); ?></span></h3>
                        <div class="notice notice-warning inline" style="margin: 10px 0; padding: 10px;">
                            <p><strong><?php _e('⚠️ ADVERTENCIA:', 'solwed-wp'); ?></strong> <?php _e('El código PHP se ejecuta directamente. Un error puede romper tu sitio web. Solo úsalo si sabes lo que haces.', 'solwed-wp'); ?></p>
                        </div>
                        <p class="description"><?php _e('Código PHP que se ejecuta en el hook "init". No incluyas las etiquetas de apertura/cierre de PHP.', 'solwed-wp'); ?></p>
        ...
        <div class="solwed-code-editor-sidebar">
            <div class="solwed-sidebar-panel solwed-panel">
                <h3><?php _e('📊 Estadísticas de Código', 'solwed-wp'); ?></h3>
                <div class="solwed-stats-info">
                    <p><strong><?php _e('Total Líneas:', 'solwed-wp'); ?></strong> 
                        <span class="solwed-status-badge <?php echo ($total_lines > 0) ? 'sent' : 'failed'; ?>">
                            <?php echo $total_lines; ?>
                        </span>
                    </p>
                    <p><strong><?php _e('CSS Normal:', 'solwed-wp'); ?></strong> <?php echo $css_lines; ?> líneas</p>
                    <p><strong><?php _e('CSS Prioritario:', 'solwed-wp'); ?></strong> <?php echo $css_priority_lines; ?> líneas</p>
                    <p><strong><?php _e('JavaScript:', 'solwed-wp'); ?></strong> <?php echo $js_lines; ?> líneas</p>
                    <p><strong><?php _e('PHP:', 'solwed-wp'); ?></strong> 
                        <span class="solwed-status-badge <?php echo ($php_lines > 0) ? 'failed' : 'sent'; ?>">
                            <?php echo $php_lines; ?> líneas
                        </span>
                    </p>
                </div>
            </div>
            ...
            <div class="solwed-sidebar-panel solwed-panel">
                <h3><?php _e('⚡ Estado Activo', 'solwed-wp'); ?></h3>
                <div class="solwed-stats-info">
                    <p><strong><?php _e('CSS Personalizado:', 'solwed-wp'); ?></strong> 
                        <span class="solwed-status-badge <?php echo !empty($editor_settings['custom_css']) ? 'sent' : 'failed'; ?>">
                            <?php echo !empty($editor_settings['custom_css']) ? __('Activo', 'solwed-wp') : __('Vacío', 'solwed-wp'); ?>
                        </span>
                    </p>
                    <p><strong><?php _e('CSS Alta Prioridad:', 'solwed-wp'); ?></strong> 
                        <span class="solwed-status-badge <?php echo !empty($editor_settings['custom_css_priority']) ? 'sent' : 'failed'; ?>">
                            <?php echo !empty($editor_settings['custom_css_priority']) ? __('Activo', 'solwed-wp') : __('Vacío', 'solwed-wp'); ?>
                        </span>
                    </p>
                    <p><strong><?php _e('JavaScript:', 'solwed-wp'); ?></strong> 
                        <span class="solwed-status-badge <?php echo !empty($editor_settings['custom_js']) ? 'sent' : 'failed'; ?>">
                            <?php echo !empty($editor_settings['custom_js']) ? __('Activo', 'solwed-wp') : __('Vacío', 'solwed-wp'); ?>
                        </span>
                    </p>
                    <p><strong><?php _e('PHP Personalizado:', 'solwed-wp'); ?></strong> 
                        <span class="solwed-status-badge <?php echo !empty($editor_settings['custom_php']) ? 'failed' : 'sent'; ?>">
                            <?php echo !empty($editor_settings['custom_php']) ? __('¡Activo!', 'solwed-wp') : __('Seguro', 'solwed-wp'); ?>
                        </span>
                    </p>
                </div>
            </div>
            ...
            <div class="solwed-sidebar-panel solwed-panel" style="border-left: 4px solid #d63638;">
                <h3><?php _e('⚠️ PHP Activo', 'solwed-wp'); ?></h3>
                <p style="color: #d63638; font-size: 12px; line-height: 1.4;">
                    <strong><?php _e('¡ADVERTENCIA!', 'solwed-wp'); ?></strong><br>
                    <?php _e('Hay código PHP personalizado ejecutándose. Asegúrate de que es correcto para evitar errores.', 'solwed-wp'); ?>
                </p>
            ...
            <div class="solwed-sidebar-panel solwed-panel">
                <h3><?php _e('💡 Consejos', 'solwed-wp'); ?></h3>
                <ul style="padding-left: 20px; line-height: 1.6;">
                    <li><?php _e('Usa CSS normal para estilos estándar', 'solwed-wp'); ?></li>
                    <li><?php _e('CSS de alta prioridad sobrescribe todo', 'solwed-wp'); ?></li>
                    <li><?php _e('JavaScript se carga en el footer (más rápido)', 'solwed-wp'); ?></li>
                    <li><?php _e('EVITA PHP personalizado si es posible', 'solwed-wp'); ?></li>
                    <li><?php _e('Haz backup antes de guardar cambios', 'solwed-wp'); ?></li>
                </ul>
            </div>

            <?php if (!empty($editor_settings['custom_css']) || !empty($editor_settings['custom_css_priority'])): ?>
            <div class="solwed-sidebar-panel solwed-panel">
                <h3><?php _e('🎨 Vista Previa CSS', 'solwed-wp'); ?></h3>
                <?php if (!empty($editor_settings['custom_css'])): ?>
                <h4 style="font-size: 12px; margin: 10px 0 5px 0;"><?php _e('CSS Normal:', 'solwed-wp'); ?></h4>
                ...
                <h4 style="font-size: 12px; margin: 10px 0 5px 0;"><?php _e('CSS Prioritario:', 'solwed-wp'); ?></h4>
